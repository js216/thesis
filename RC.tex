...
\begin{figure}
    ...
    \caption{Residual microwave reflectivity when the window ($t=11.494$~mm) is
    not of the optimal thickness.}
    \label{AR_win_p2}
\end{figure}
...
\begin{figure}
    ...
    variations, for window thickness $t=11.494$~mm.}
    \label{AR_win_p3}
\end{figure}
...
\begin{figure}
    ...
    \caption{Residual reflectivity versus microwave frequency variations, for
    window thickness $t=11.494$~mm.}
    \label{AR_win_p4}
\end{figure}
...
have the beam radius of curvature and width
...
\begin{figure}
    ...
    reflectivity due to wavefront curvature.}
    \label{AR_win_p6}
\end{figure}

The outer parts of the beam will be reflected much more than the beam
centre. However, the beam intensity near the outer parts of the window is
greatly reduced since most of the beam power is near the centre. At angles where
we would predict a high reflectivity, the Gaussian is exponentially suppressed.
Thus, the actual power reflected from the edges of the windows due to the beam
...
the beam axis. If we want the intensity at the edge of the windows to be 0.25\%\
of the intensity at the center of the beam, then the window diameter $D$ needs
to be
%
\[
...
\]
%
where the beam width is given in eqn~\ref{radius_width}. The vacuum chamber as
built (Fig.~\ref{RCch}) has three pairs of identical microwave windows. The two
holes for 13.4~GHz microwaves have a diameters of 8.150" and are separated by
14.58", while the other two pairs (for 26.8 and 40.1~GHz) both have a 5.450"
...
all the laser and microwave beams.
...
\subsubsection{Beam-blocking mesh}
...
conductivity $\sigma=1/\rho$, arranged in an array with spacing $d$. The
reflection coefficient for radiation of frequency $\omega=2\pi f$ and wavelength
$\lambda=c/f$ at normal incidence is \cite[eqns~24--25]{wait1955reflection}
%
\[
...
\end{align*}
%
The transmission $T\approx1-|R|^2$ of 13.4~GHz microwaves through a grid is
shown in Fig.~\ref{wires}. The resistivity of the austenitic (non-magnetic) 304
stainless steel is $\rho=72~\mu\Omega\cdot\text{cm}$
\cite[p.~12-234]{haynes2016crc}, while the permeability is close to the vacuum
value ($\mu_0\approx4\pi\cdot10^{-7}$~H/m).

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figs/mathematica/wires.pdf}
    \caption{Transmission of 13.4~GHz microwaves at normal incidence through a
    wire mesh.}
    \label{wires}
\end{figure}
