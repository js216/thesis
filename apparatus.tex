...
delivered to a second \gls{SHG} system (Toptica SHG pro) with a similar
...
\begin{figure}
    ...
    26.8~GHz. For 40.1~GHz, the system is adjusted to generate approx.\ 10~GHz
    (pre-multiplier).}
    \label{micro}
\end{figure}
...
factor of $S$, called selectivity and expressed in dB. The maximum phase noise
...
beam axis. The two transitions are driven with two microwave beams that are
offset by approx.\ 3~cm along the beam direction.

\subsection{Electrostatic lens}

\textit{All work done by OT; no contribution from JK.}
...
distinct rotational manifolds, and from there, read out using laser-induced
fluorescence.
