...
design of a conventional molecular beam experiment. Fig.~\fref{entire} displays
...
construction. Italicized notes at the beginning of each subsection credit the
main people responsible for implementing each subsystem.

\begin{figure*}[b!]
    \includegraphics[width=\textwidth]{figs/fig5overview.pdf}
    \caption[Overview of experimental apparatus.]{Overview of the \CENTREX\
    beamline without the magnetic shield around the interaction region. The
    horizontal ordinate is given in meters. This is Fig.~5 from our overview
...
\begin{table}[!htb]
    ...
    \begin{tabu} to \columnwidth {@{}rcX[c]l@{}}
        ...
        \\\multicolumn{3}{@{}l}{\textit{Detection region:}}\\
           detection efficiency         & $\eta_\text{det}$ & 30~\%                 & ---              \\
...
\textit{Concept and implementation of the base code, JK. Additional features,
upgrades, bug fixes, and the majority of device drivers, Olivier Grasdijk. (As
of Aug 30, 2023, JK added/removed 126,067/115,315 lines of code, and OG
45,198/16,949.)}
...
obtained data to the local disks (see Fig.~\fref{DAQ_flow}). In the future, we
intend to set up a common data and backup server to deal with the greatly
increased data generation rate once the entire apparatus is up and running.

\begin{figure}
   \centering
  \includegraphics[width=\textwidth]{figs/DAQ.pdf}
   \caption{Experiment data flow.}
  \label{DAQ_flow}
\end{figure}
...
The control of devices via the \verb`Device` objects is asynchronous, such that
the timing of operations is not precisely controlled. For triggered experimental
sequences, where the precise timing and sequence is important, we use a hardware
TTL pulse generator (PulseBlaster from SpinCore Technologies, Gainesville, FL).

Warnings and error conditions are reported to the \verb`warnings` list inside
each \verb`Device` object. These conditions are reported to the user through the
\verb`Monitoring` class and written to the hard drive.
...
description in terms of arbitrary attributes, column names, and groups. The
write speed is comparable to writing raw data to the hard drive
\cite{malaya2010petascale}.
...
\verb`Config` class. The latter in turn only allows to set the attributes that
...
\textit{Entire design and implementation, JK. However, see
sect.~\ref{beam_source_history} for the origin of the dual pulse tube beam
source idea. See also entire chapter \ref{beam_source_ch} for a more complete
discussion of the beam source.}
...
window through which the ablation laser (Fig.~\fref{ablationLaser}) enters from
...
\begin{figure}
    ...
    \caption[Ablation laser system.]{The ablation laser for the beam source is a
    pulsed Nd:YAG, providing 4~ns pulses of maximum energy 130~mJ at max.\
    50~Hz (Nano LG 130-50 by Litron Lasers Ltd, UK). In beam source tests, we
    have commonly run it such that the pulse energy is approx.\ 35~mJ, at a
    repetition rate of 5--10~Hz. The photodiode receives a small amount of light
    split off the main beam, and is used for monitoring the laser power.
    [Drawing by O. Grasdijk]}
    \label{ablationLaser}
\end{figure}
...
radiation by a two-stage heat shield. The outer shield, made of aluminum 1100,
...
\begin{figure*}
    ...
    \caption[UV laser system.]{UV laser system (see text). \part{a} Cs reference
    laser. \part{b} Transfer scanning cavity frequency lock. \part{c} Two-state
    \gls{SHG}. [Drawings by O. Grasdijk]}
    \label{lasers}
\end{figure*}

Cell windows are standard fused-silica \UV-grade windows (from Thorlabs, Newton,
NJ). The anti-reflection coatings, done at 271~nm (excited-to-ground molecular
...
conductivity has been enhanced either with indium foil (99.99~\%\ pure, 0.002"
thick, p.n.\ ribbonin-10101 from Indium Corporation, Utica, NY), or
vacuum-compatible grease (Apiezon N).

\subsection{Pumping system}

\textit{System mostly assembled by OG, with help from JK and others.}

In addition to providing thermal isolation, the shields also serve as cryopumps,
lowering the pressure from $\sim10^{-6}$~torr, as can be reached by our
...
\textit{Current mounts for the beam source, RC/SPA, interaction region, as well
as the transverse-rail system, conceived of and implemented by JK. Original beam
source mount by OG, and most of the lens mount built by Oskari Timgren. Other
mounts TBD.}
...
out of the beamline perpendicularly (Fig.~\fref{transverse_rail}). This is
...
\begin{figure}
  ...
  \includegraphics[width=\textwidth]{figs/todo/transverse_rail.png}
   \caption[Transverse rail mounting structure.]{Transverse rail mounting
    ...
    exception of the interaction region, are to be mounted similarly. The
    beamline direction is approximately from left to right. See text for details
    of the setup, e.g.\ the available translation distance.}
  \label{transverse_rail}
\end{figure}

The aluminum rails transverse to the beamline direction, as shown in
Fig.~\fref{transverse_rail}, are 4' long. Given the size of the mounting
structures underneath the vacuum chambers, this is enough to translate the
chamber transverse to the molecular beam by a couple of inches. When a vacuum
chamber needs to be moved entirely out of the way of the surrounding modules,
the transverse rails are extended by another set of two 4' pieces per module.
This way, the chamber can be moved fully four feet away from the beamline
region. The extension pieces are not normally attached since they are a trip
hazard and impede the motion of apparatus and people around the room.

\subsection{Tunable laser system}

\textit{Majority of laser design, implementation, and maintenance done by OG.
Some earlier versions by Konrad Wenz. No contribution from JK except for the
laser power stabilization proof of concept, appendix~\ref{laser_power_control}.}

See Fig.~\fref{lasers} for a diagram of the tunable \UV\ laser system. It
consists of three systems working together to (1) provide a stable cesium
reference line, (2) lock an \IR\ seed laser to the reference, and (3) to
quadruple the \IR\ into a \UV\ beam that can be used to probe transitions in
TlF, occurring around 271~nm. In order to provide separately-controllable laser
frequencies, we have three \glsreset{SHG}\gls{SHG} systems connected to two
transfer lock cavities (explained on page \pageref{transfer_cavity}), one of
...
the counter-propagating probe beam, passed through the same vapour, through the
...
acousto-optic modulator (\AOM) (AOMO 3080-122 830NM from Gooch \&\ Housego,
England, UK) with 80~MHz center frequency, AR-coated for operation at
780--850~nm, into a pump and probe beam, the pump having about twice the power
of the probe. (The \AOM\ also eliminates trouble from back-reflection from the Cs
cell, or photodiodes.) The pump beam is phase-modulated by the custom-built
\EOM\ (Thorlabs Inc.) at 3~MHz before being led through the cesium quartz cell
...
negligible compared to the $\approx0.1$~MHz stability required for the \IR\
science lasers.

\paragraph{Transfer cavity}
\label{transfer_cavity}

Fabry--P\'erot interferometer (Toptica FPI 100) with free spectral range
...
slightly more than one \FSR\ to scan over the transmission peaks. The timing
difference between the master and slave peaks is then fed to a software-defined
\PID\ loop to lock the slave frequency.
...
(built by K. Wenz at Columbia University, NY) containing a ppKTP crystal that
generates up to 1~W of green light at 543.5~nm. The green beam is coupled into a
fiber and delivered to a second \gls{SHG} system (Toptica SHG pro) with a
similar configuration. A BBO crystal is used for frequency doubling, resulting
in $\approx70$~mW of \UV\ (271.7~nm).
...
\textit{Vacuum chamber design and initial purchasing/assembly of microwave
electronics by JK. Subsequent work by OG.}
...
The microwave system (see Fig.~\fref{micro}) is designed to provide up to three
microwave beams (at frequencies 13.4, 26.8, and 40.1~GHz, and power of order
0.1--1~W), each focused with a quasi-optical lens built into the transmitting
...
\begin{figure}
    ...
    \caption[Frequency-agile microwave generation.]{The principle behind
    frequency-agile rotational cooling microwave generation. The frequencies are
    shown assuming the $\times1,2$ multipliers for a horn output of 13.4 and
    26.8~GHz. For 40.1~GHz, the signal generator is adjusted to generate
    approx.\ 10~GHz before it is multiplied $\times4$.}
    \label{micro}
\end{figure}
...
frequency $f_0$ is used to down-convert the desired signal $f_\text{D}$ to an
intermediate frequency $f_\text{IF}$. We want to know what is the maximum
allowable phase noise $L_\text{max}$ (in dBc/Hz) so that the down-conversion of
an adjacent undesired (interference) signal $f_\text{ID}$ is suppressed by a
factor of $S$, called selectivity and expressed in dB. The maximum allowable
phase noise is \cite[eqn~13.50]{pozar2006microwave}
%
\[
...
The \DDS\ source is AD9910 (Analog Devices Inc., Norwood, MA), outputting to
...
one of the 100~MHz sidebands from a signal exceeding 10~GHz.) The resulting
frequency-agile microwave signal is doubled or quadrupled if necessary,
amplified, and directed by the SPDT switch into one or the other input ports of
the orthomode transducer and led to the horn. The horns were custom engineered
by Sage Millimeter (now Eravant Inc., Torrance, CA) for spot-focusing the beam
energy onto the molecular beam.

\subsection{State preparation A}

\textit{Vacuum chamber design by JK; all other work done by OT.}

The SPA chamber exists to transfer molecular population from the $\ket{J=0,F=0}$
state populated during rotational cooling to the
$\ket{J=2,m_J=0,I_\text{tot}=0}$ state that can be efficiently focused by the
electrostatic lens. \cite[ch.~7]{timgren2023thesis}. This is done by applying
the adiabatic passage technique \cite{treacy1968adiabatic} twice in succession.
First, the molecules are transferred from $J=0$ to $J=1$, and then from $J=1$ to
$J=2$. The molecular beam passes through the middle of two ring-shaped
electrodes applying the correctly spatially varying electric field along the
beam axis to achieve Stark-tuned adiabatic passage for the two transitions. The
two transitions are driven with two microwave beams that are offset by approx.\
3~cm along the beam direction.

\subsection{Electrostatic lens}

\textit{Lens design, construction, and initial high-voltage testing by OT.}

The lens \cite[ch.~8]{timgren2023thesis} consists of four rod-shaped electrodes
arranged in a quadrupole configuration. The 23.622" long rods are manufactured
out of 316-alloy stainless steel with circular cross-section, diameter 1.75",
and ends rounded into half-spheres to avoid high electric field concentration
that would result in sparking. The support rods holding the electrodes are
machined out of MACOR ceramic. The electrodes--rod assembly is held in place by
two stainless rings. Each adjacent pair of electrodes has a 2.475" separation,
such that a circle inscribed between the four electrodes, representing the clear
aperture through the lens, has a 1.75" diameter. The pair of electrodes
diagonally opposite each other have the same voltage of magnitude 30~kV; the two
pairs have opposite polarity. The vacuum chamber is a 16" OD, 24.6" long
cylinder made of 1/8" thick stainless steel, with ISO400F flanges at each end
and a sequence of smaller ports for electrical feedthroughs, pumps, windows,
etc.

\subsection{State preparation B}

\textit{All work done by OT and Jianhui Li; no contribution from JK.}

The goals of the SPB section \cite[ch.~9]{timgren2023thesis} is to drive
molecules from the lens state to a state in the $J=1$ manifold that is suitable
for the Schiff moment measurement.  Much like the SPA region, SPB utilizes
adiabatic passage with a spatially-varying electric field, and circularly
polarized microwaves to select the desired $m_J$ state. This time, only a single
adiabatic passage transition is needed since $|\Delta J|=1$, and it is again
driven by a Gaussian microwave beam. The beam passes through a pair of
ring-shaped electrodes, while the molecular beam goes between the two rings in
contrast to the SPA region.

\subsection{Interaction region}

\textit{Vacuum chamber and electrodes by Tristan Winick and David Kawall.
Magnetic shield and early work on the high-voltage system by JK.}

The interaction region is a glass tube (26~cm OD, 350~cm length, wall thickness
10~mm) terminated with custom flanges at each end serving as the vacuum chamber
for the beamline. Inside the chamber, there are two parallel plate electrodes,
approx.\ 15~cm wide by 3~m long, separated by exactly 2~cm and held in place
using a series of MACOR rings and rods. The vacuum chamber is enclosed in the
magnetic field control system (see section \ref{mfc_chapter}), consisting of the
magnetic shield and the shim coils.

\subsection{State preparation C and readout}

\textit{Design and implementation by JL; no contribution from JK.}

The SPC and readout chambers are currently under construction. Their purpose is
to transfer the population from the superposition of hyperfine states into two
distinct rotational manifolds, and from there, read out the rotational state
populations using laser-induced fluorescence.
...
In this chapter, we have provided an overview of the experimental layout of
\CENTREX. The reader is encouraged to refer back to this chapter if unsure about
how a certain module discussed later fits into the greater scheme of the
experiment.

\keypoint{Why is it important for CeNTREX?}

The work of the \CENTREX\ collaboration is primarily experimental. The segments
of the experiment spanning from the beam source to the final detection chamber
may, when fully constructed and operational, constitute one the most significant
contributions to precision measurements of $T$-violating interactions to date.

\keypoint{Will we revisit it in later sections?}

Every chapter following this one will expand on details related to one or more
of the modules of the experiment. While some review will be provided in later
sections, it can be helpful to periodically peruse this overview of the
apparatus as we proceed with the discussion.

\keypoint{What are the biggest remaining challenges to getting a measurement?}

Most of the modules have already been constructed with the exception of
State preparation C. Furthermore, a number of the modules (notably SPB, SPC, the
lens, the interaction region, SPC, detection region) require in-depth testing
before they can be integrated into the beamline. This work is actively being
