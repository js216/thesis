...
This section describes some of the electronic circuits we have designed and built
...
\begin{itemize}
   ...
   \item Relays are highly inductive which can cause high voltage transients if
      not properly protected. In all circuits involving relays, we use a power
...
The LaserPIDpower circuit (schematic in Fig.~\fref{laserPIDpower_schem}, PCB in
Fig.~\fref{laser_PCB}) reads a photodiode voltage and outputs an 80~MHz RF signal
...
The Digital isolator circuit (schematic in Fig.~\fref{digitalIsolator_schem}, PCB
in Fig.~\fref{laser_PCB}) provides high-voltage isolation for four signal lines,
...
The Shim coils circuit (schematic in Fig.~\fref{shimcoils_schem}, PCB
in Fig.~\fref{coils_PCB}) is a 16-channel bipolar approx.\ $\pm10$~mA current
...
the global chip shortage. We have thus adapted the board to the two 8-channel
DACs, and in order to avoid having to change the digital isolator to add another
CS line, we generate it using an inverter (U38, SN74LVC1GU04DRLR).
...
The circuit board includes a BNC connector J4 connected to a precision
100~$\Omega$ resistor R18 (visible in Fig.~\fref{coils_PCB}, left, just above the
Arduino) of the same kind as the resistors used in the part of the circuit that
generates the constant current. By connecting the output of any of the
current-source channels to the resistor, we can test the stability of the output
current by measuring the voltage across the resistor, available at the BNC
connector J4. With one of the current-source channels set to midscale, we have
observed this voltage over three weeks as shown in Fig.~\fref{shim_stability}
(top part).

The bottom row of Fig.~\fref{shim_stability} shows a different way to measure the
stability of the current source. The circuit was connected to the shim coils, as
is done using normal operation. However, a precision multimeter was inserted
between the current source and the coils using an adapter board. The plots show
a few days' worth of measurement at two output settings. The midscale
measurement, where the board is effectively trying to output no current, shows a
drift of the order of 25~nA over a week. The third-scale output of approx.\
3.8~mA shows a drift of 100~nA over three days. To put these numbers into
perspective, note that the shim coil response is approx.\ 40~mG/A. Since we
would like a long term stability of coil field better than 0.1~$\mu$G, we thus
need better than 2.5~$\mu$A current-source stability, which is well within the
actual performance of the circuit.

\begin{figure}
   \centering
  \includegraphics[width=\textwidth]{figs/todo/SR560_stability.png}
  \includegraphics[width=\textwidth]{figs/todo/DMM_stability.png}
    \caption[Stability measurement of the shim coils current source.]{Stability
    measurement of the shim coils current source. \part{Top row} The voltage
    shown was developed across resistor R18 with the current source set to
    midscale, amplified by a low-noise voltage preamplifier (Stanford Research
    Systems SR560), and recorded by the moku:lab data logger. \part{Bottom row}
    Current stability directly measured using a precision DMM (Keysight 3458a)
    inserted between the current source and one of the shim coils.}
  \label{shim_stability}
\end{figure}

\paragraph{Relay board}

The Shim\_relays circuit (schematic in Fig.~\fref{shimrelays_schem}, PCB
in Fig.~\fref{coils_PCB}) is placed between the shim coils and the current source
...
Fig.~\fref{degaussingrelay_schem}, PCB in Fig.~\fref{deg_PCB}) is used to
...
The Degaussing DAC board circuit (schematic in Fig.~\fref{degaussingDAC_schem},
PCB in Fig.~\fref{deg_PCB}) is used to generate a decaying sinusoidal signal to
...
The ADC board circuit (schematic in Fig.~\fref{decapsuADC_schem},
PCB in Fig.~\fref{deca_PCB}) is used to digitize up to five 3-axis sensor
...
The preamp board circuit (schematic in Fig.~\fref{decapsuPreamp_schem},
PCB in Fig.~\fref{deca_PCB}) can be used between the DecaPSU unit and the ADC
...
The magnetometer boards (schematic in Fig.~\fref{magnetometer_schem},
PCB in Fig.~\fref{multiADC}) consist simply of an RJ45 connector and three
...
(schematic in Fig.~\fref{multiadc_schem}, PCB in Fig.~\fref{multiADC}) integrates
...
The HV\_control circuit (schematic in Fig.~\fref{HVctl_schem}, PCB in
Fig.~\fref{HV_pic}) enables computer control of the high voltage power supply as
...
resistor potted in a grounded aluminum enclosure (see section
\ref{volt_meas_sect}, Fig.~\fref{resistor_box}). This resistor converts the
