...
100~$\Omega$ resistor R18 (visible in Fig.~\ref{coils_PCB}, left, just above the
...
observed this voltage over three weeks as shown in Fig.~\ref{shim_stability}
(top part).

The bottom row of Fig.~\ref{shim_stability} shows a different way to measure the
...
resistor potted in a grounded aluminum enclosure (see Fig.~\ref{resistor_box}).
This resistor converts the 30~kV output voltage to a 3~$\mu$A current, which is
measured on the HV\_control PCB as an approx.\ 3~V voltage across a precision
1~M$\Omega$ resistor (R19, Caddock USF370-1.00M-0.1\%-5PPM). Thus a 1:10,000
voltage divider is set up by the potted 10~G$\Omega$ resistor and the
1~M$\Omega$ resistor on the PCB.
