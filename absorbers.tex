...
following, we present several types of radiation-absorbing sheets, then evaluate
...
\end{align*}
%       
These expressions are plotted in Fig.~\fref{simplest}, where it is evident that
...
this approach could be implemented in practice. Fig.~\fref{salisbury} shows the
...
In a typical microwave metamaterial absorber (see Fig.~\fref{MA} for examples), a
...
In the following, we will discuss three ways of looking at metamaterial
...
will show what we consider more enlightening ways to understand the underlying
physics.
...
losses. For example, in a microstrip ring resonator (see Fig.~\fref{ring}, as
...
(Fig.~\fref{grating}). The greater the number of antenna elements, the sharper
...
metamaterial absorber, in the following section we consider how much of an
...
To define the problem more precisely, consider Fig.~\fref{quarter} showing a
...
Fig.~\fref{magnE} on a logarithmic scale. The simulation is repeated several
