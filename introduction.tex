...
\begin{figure}
    ...
    length $L=10l$. \part{a} Comparing \SOF\ ($\tilde b_\text{\SOF}=0.600$,
    $\phi=0$) and Rabi oscillations ($\tilde b_\text{Rabi}=1.200$), both with
    the same $L$. Notice the $\approx40$\%\ narrower \FWHM\ for \SOF. \part{b}
    Detection scheme sensitive to small $\delta$: \SOF\ with $\phi=\pi/2$ with
    the same $L=10l$.}
    \label{SOFprob}
\end{figure}
...
\begin{figure}
    ...
    \SOF\ signal as a function of detuning $\Delta f$ in linear units, averaged
...
\begin{table*}
    ...
    \makebox[\textwidth][c]{%
        ...
        previous \gls{EDM}-related experiments.}
        \label{previouswork}
\end{table*}
...
\begin{sidewaysfigure}
    ...
    (below) and the quantum states (above).}
...
\begin{sidewaystable}
    ...
    The labels $l_\text{R}$ are as given in ref.~\cite{wilkening1984search}. The
    column labelled $S_\text{\gls{SMT}}$ gives the sensitivity to the Schiff
    moment (eqn~\ref{ssmt}). The quantity $S^z_m$ gives the energy difference
    between the neighbouring resonances, given per unit external residual $B_z$,
    and corresponds to flipping the internal molecular magnetic field. Due to
