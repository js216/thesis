...
of opposite parity (Fig.~\fref{polarize}a). The two states are separated in
...
the combined wavefunction (Fig.~\fref{polarize}b), for the lowest energy state.
...
\]
%
as in Fig.~\fref{graddelta}a. Taking the derivative of the Gaussian, we get a
shape that is negative for $r<0$, positive for $r>0$, zero at $r=0$ and also
vanishing for $r\to\pm\infty$. That shape (Fig.~\fref{graddelta}b), stretched to
...
shape (Fig.~\fref{graddelta}c) as the expression $\nabla\delta(\vec R_0)$
...
$\bra\psi\cdots\ket\psi$. See Fig.~\fref{polarizeTlF} for a plot of $\mathcal{P}$
...
\subsection{EDM measurements in a molecular beam}
...
\]
%
the two choices of $\phi$ are depicted in Fig.~\fref{SOFprob}. The above numbers
...
It will be noticed that there are high-slope regions in Fig.~\fref{SOFprob} for
...
$f_\text{BG}(v)\to0$ for $v$ far away from $\overline v$. See Fig.~\fref{SOF2}
...
\]
%
as displayed in Fig.~\fref{SOF2}a. For the parameter values in that figure, the
...
sources, the \mfp\ is larger than or comparable with the linear dimension $d$ of
the hole through which the molecules emerge from the beam source. The particles
escape by chance, and the beam properties reflect the state of the molecules in
the oven. By contrast, in \emph{jet sources}, the \mfp\ is much smaller than
$d$. The beam properties no longer reflect those of the oven molecules; instead,
the local thermal (electronic, rotational, and vibrational) energy is partly
converted into the center-of-mass motion of the particles. These beams are thus
faster but colder.
...
to exist as a monomer, the rest being mostly dimers (TlF)$_2$.
\cite{schropp1987new,schropp1988thesis} In addition to decreasing the useful
signal, dimers were twice as effective at producing background counts with the
detection technology used \cite{schropp1987new,schropp1988thesis}. However, by
Le Chatelier's principle the equilibrium of the endothermic reaction
$\text{(TlF)}_2\rightleftharpoons2\text{TlF}$ can be shifted towards the right
by an increase in temperature or a decrease in pressure. Thus the oven in
\cite{schropp1987new,schropp1988thesis} consists of two stages: a lower,
high-pressure section with boiling TlF salt, and an uppper, lower-pressure
region, separated by a constriction. As a result, the monomer fraction may be as
high as 50\%\ in the two-chamber design.
...
The recent trend is towards cryogenic buffer-cooled beam sources
\cite{hutzler2012buffer}. A non-reactive buffer gas is flowed through a copper
cell kept at a temperature $T_0$. The flow rate $F$ of the buffer gas controls
the numbre density of the buffer gas in the cell. The species of interest is
injected at a temperature much higher than $T_0$ but thermalizes with the buffer
gas, cooling its rotational and translational degrees of freedom to
approximately $T_0$. Since the cell is at a cryogenic temperatures, target
molecules that collide with the walls generally stick to it, while others escape
through an exit aperture in the cell; the ratio of the time scales of these two
processes determines the extraction efficiency of the cell. Beam properties are
largely determined by the collisions between the target particles and the buffer
gas around the exit aperture. See \cite{hutzler2012buffer} for a review of
...
hole in a 0.5~mm thick copper plate. The beam source design was adapted by
...
only the transitions between the rotational states of the $v=0$ vibrational
sublevels of the electronic states $X^1\Sigma^+$ and $B^3\Pi_1$, where $X$
denotes the ground state, and $B$ is one of the excited states with a lifetime
of about 99~ns \cite{norrgard2017hyperfine}. The quantum numbers used to
describe the states usually refer to a modified Hund's case (c) basis
\cite{brown2003rotational} with the additional couplings $\vec F_1=\vec J+\vec
I_1$ and $\vec F=\vec F_1+\vec I_2$, where $\vec I_1$ and $\vec I_2$ refer to
thallium and fluorine nuclear spins, respectively; both nuclei are spin-half.
...
\end{align*}
%
where $\Omega$ is the projection of $\vec J$ on the internuclear axis and $m_F$ is the
projection of $\vec F$ in the laboratory frame. \cite{norrgard2017hyperfine} The
...
Fig.~\fref{diagram} in the absence of electric and magnetic fields. The largest
effect is the rotational energy $\H_\text{rot}=hB\vec J^2$ with the rotational
constant of roughly $2B=13.4~\GHz$. More precisely, the rotational-vibrational
...
eigenenergies significantly. However, it induces mixing between $J=0$ and $J=2$
states which results in the slope of the \gls{NMR} resonance frequency
as a function of the applied field being a factor of $\approx2$ greater when using
eqn~\ref{newc3}, as will be seen in Fig.~\fref{Cslope} on page \pageref{Cslope},
reproducing the result 41~mHz/(V/cm) reported at 20~kV/cm in Ramsey et al.\
\cite{wilkening1984search}, while the form of the Hamiltonian as given in that
paper gives only 20.2~mHz/(V/cm). The later Hinds et al.\ paper
\cite{cho1991search, cho1990thesis} seems to have copied the slope from Ramsey,
claiming it holds at 29.5~kV/cm, where the correct slope (as per
eqn~\ref{newc3}) would be 31.5~mHz/(V/cm).
...
$|d_\text{n}|<2.9\times10^{-26}e\cm$. Since the particles were kept in a trap,
...
experiments. \cite{romalis2001new}. The result yields a constraint of
...
Referring to Fig.~\fref{conceptdiagram}, we follow the molecules emerging from a
...
\paragraph{Rotational cooling (Fig.~\fref{scheme})}
...
\begin{sidewaystable}
    ...
    Fig.~\fref{cho1991fig7}, above. The column labelled $S_\text{\gls{SMT}}$
...
Refer to Table \ref{statepairs} and Fig.~\fref{cho1991fig7} for a list of all
...
These preliminary discussions are important to understand the \CENTREX\
experiment. Despite its small scale compared to typical high energy physics
experiments, our experiment, which consists of a number of modules relying on
modern atomic and molecular physics techniques, can provide information highly
...
the \CENTREX\ collaboration. On the level of a high-level overview as seen here,
