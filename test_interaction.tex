\section{1/3-scale Interaction Region}
\label{test_int_ch}

The main interaction region assembly is several meters long and contains
...
make a scaled-down version of it, described in this section.
...
tube outer diameter is the same as for the full-scale interaction region vacuum
tube, and the flanges are the same pieces that will be used with the full-scale
vacuum chamber. Testing with helium revealed no leaks at the flanges.

\subsection{Electrode assembly}

The two electrodes (Fig.~\fref{test_elec}) were machined out of grade 5 titanium
...
\begin{figure}
    ...
    \caption[Test interaction electrode assembly.]{Electrode assembly for the
    test interaction region before being placed in the vacuum chamber.}
    \label{test_elec}
\end{figure}
...
very low force acting to `peel off' the joint (Fig.~\fref{epoxy_forces}). After
...
\begin{table*}
  ...
  \makebox[\textwidth][c]{%
    \begin{tabu} to \textwidth {rcc}
      ...
      \bottomrule
    \end{tabu}}
    \caption[Shear stress and peel-off force of epoxies.]{A comparison of shear
    stress and peel-off force requirements and epoxy performance (from their
...
The magnetic shield (Fig.~\fref{test_shield}) is constructed around a single
schedule 40 PVC pipe, 14" outer diameter, 0.46" wall thickness, 4~ft long, which
has been wrapped with two layers of Co-netic foil, 0.254~mm thick. The foil
comes in 15" wide sheets; three such sheets (with 1" overlaps) were wrapped
circumferentially around the plastic tube. In addition, 12 turns of a degaussing
wire were wrapped in a toroidal fashion around the shield. The static residual
field inside the shield along the central axis is shown in
Fig.~\fref{test_shield_field}.
...
In this short chapter, we have presented our 1/3-scale interaction region, its
vacuum chamber, electrode assembly, and the magnetic shield.

\keypoint{Why is it important for CeNTREX?}

The scaled interaction region will be useful to prove the operation of the
experiment without the difficulty of setting up the extremely delicate, as well
as large, full-scale interaction region. This will allow us to determine that
the different modules of the experiment are able to work together as a unit.
When the full-scale interaction region will be ready, it can be simply swapped
in to begin the final measurements.

\keypoint{Will we revisit it in later sections?}

In the following chapter, we will discuss the high-voltage system that can be
used to supply the $\pm30$~kV for the test interaction region electrodes. (We
have placed the high voltage into a separate chapter, since the electrostatic
lens, a separate module, also requires a power supply with the same voltages.)

\keypoint{What are the biggest remaining challenges to getting a measurement?}

The 1/3-scale interaction region needs to be tested under high voltage for
sparking and x-ray generation before it can be safely used in the experiment.
The experience gained from manipulating the scaled version will be useful in
operating the full-sized apparatus.
