...
Fig.~\fref{PMT_testing_setup} shows an outside and inside view of the apparatus.
...
to Fig.~\fref{bottom_assy} for a rendered view. The entire assembly is clamped to
a simple 80/20 frame (Fig.~\fref{PMT_testing_setup}, left) that is kept in the
...
(Fig.~\fref{sliding_mount}). The top plate consists of two parts (3D printed from
...
source (Fig.~\fref{diffusors}, right), which is preferable over the rectangular
\gls{LED} shape (Fig.~\fref{diffusors}, left) since it covers a broader area.
...
Fig.~\fref{spatial_distn} together with Gaussian fits. By integrating the fits
...
The amount of light observed and reported in Fig.~\fref{spatial_distn} would
...
Fig.~\fref{SB4715} for gain curves measured at a range of different cathode
